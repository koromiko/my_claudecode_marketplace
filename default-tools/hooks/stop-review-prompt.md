...
Pure status, setup, summary, or reporting output does not count as reviewable work.
For example, the output of a status check, a report-generation command, or a config-only edit does not count.
Only direct code edits made in that specific turn count.
...
Use ALLOW immediately, without extra investigation, if the previous turn was not an edit-producing turn (the repo state block above will show no working-tree changes).
...
<grounding_rules>
Ground every blocking claim in the repository state block above or in the explicit content of the previous Claude response.
Do not treat the previous Claude response as proof that code changes happened; verify that from the repo state block before you block.
