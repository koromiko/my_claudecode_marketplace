...
Produce a visualized HTML companion to the existing terminal report (`default-tools/scripts/usage-report.sh`) that aggregates decisions logged to `~/.claude/logs/auto-approve.log` by the auto-approve hook.
...
- New analytics in Phase 1. Only data already aggregated by `usage-report.sh`.
- Refactoring `usage-report.sh` or extracting shared aggregation libraries.

## Artifacts

1. **`default-tools/scripts/auto-approve-report.sh`** — bash script. Reads the log files, aggregates with `awk`, emits a single self-contained HTML file.
2. **`default-tools/commands/auto-approve-report.md`** — slash command (`/auto-approve-report`) that invokes the script with `--open` and forwards `$ARGUMENTS` for time-filter flags.
3. **`default-tools/scripts/vendor/chart.umd.min.js`** — Chart.js v4 (UMD minified, ~200KB), committed to the repo so the report works offline and renders deterministically without a CDN dependency.
...
**Default time window:** last 7 days. Mirrors `usage-report.sh` flag names but differs on default (which is all-time there).
...
~/.claude/logs/auto-approve.log (+ .1 .2 .3 rotated)
  ...
  awk aggregation in auto-approve-report.sh
        │ (same patterns as usage-report.sh)
        ▼
...
- **Duplicated aggregation logic** with `usage-report.sh`. Acceptable: both scripts are <300 lines, no third caller is anticipated, and a shared library would need its own contract. Revisit only if a third report appears.
...
bash default-tools/scripts/usage-report.sh --days 7
...
- No log files at all → exits 0 with a stderr message, no HTML written, same as `usage-report.sh`.
