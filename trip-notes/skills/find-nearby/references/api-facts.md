...
|---|---|---|---|
| WALK | yes | 625 (as 1 origin × 625 destinations; not independently pushed past 625 since the origins×destinations product cap is mode-independent) | `Request exceeded the maximum number of elements. The product of the number of origins and destinations must be <= 625.` (verified on DRIVE at n=626; the error is about total element count, not mode-specific, so it applies identically to WALK) |
